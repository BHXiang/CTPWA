...
void AmpCasDecay::computeSLAmps(const std::map<std::string, std::vector<LorentzVector>> &finalMomenta)
{
    ...
    cudaMalloc(&d_amp_buffer, nEvents_ * nSLCombs_ * amp_size * sizeof(thrust::complex<double>));
                                   ...
                                   int buffer_size_per_event)
{
    ...
    for (int slIdx = 0; slIdx < num_sl; ++slIdx)
    {
        ...
        // d_amp按
        thrust::complex<double> *event_final_amp = &d_amp[slIdx * num_events * num_polar + eventIdx * num_polar];
...
//     const SL *slCombinations,              // SL组合数据
//     const thrust::complex<double> *slamps, // SL振幅
//     const DeviceResonance resonance,       // 共振信息
//     const DecayNode *decayChain,           // 衰变链信息
//     int decayChain_size,
//     int nEvents,
//     int nSLComb,
...
//         // printf("Event: %d, p1(%f, %f, %f, %f), p2(%f, %f, %f, %f) \n", event_idx, pDaug1.E, pDaug1.Px, pDaug1.Py, pDaug1.Pz, pDaug2.E, pDaug2.Px, pDaug2.Py, pDaug2.Pz);

//         double mm = pMother.M();
//         double qq = std::sqrt((mm * mm - std::pow(pDaug1.M() + pDaug2.M(), 2)) * (mm * mm - std::pow(pDaug1.M() - pDaug2.M(), 2))) / 2 / mm;

//         double mass_mother = decayChain[nodeIdx].mass[0];
//         double mass_daug1 = decayChain[nodeIdx].mass[1];
//         double mass_daug2 = decayChain[nodeIdx].mass[2];

//         if (mass_mother == -1 && decayChain[nodeIdx].mother_idx == resonance.particle_idx)
//         {
//             mass_mother = resonance.params[0];
//         }
//         if (mass_daug1 == -1 && decayChain[nodeIdx].daug1_idx == resonance.particle_idx)
//         {
//             mass_daug1 = resonance.params[0];
//         }
//         if (mass_daug2 == -1 && decayChain[nodeIdx].daug2_idx == resonance.particle_idx)
//         {
//             mass_daug2 = resonance.params[0];
//         }
...
//         if (nodeIdx == 0)
//         {
//             resAmp *= BlattWeisskopf(1, 0.1, 0.2);
// 						continue;
//         }

//         if (resonance.type == ResModelType::BWR)
//         {
//             resAmp *= BreitWigner(mm, resonance.params[0], resonance.params[1], sl.L, qq, q0);
//             resAmp *= BlattWeisskopf(sl.L, qq, q0);
//         }
//         else if (resonance.type == ResModelType::ONE)
//         {
//             resAmp *= BlattWeisskopf(sl.L, qq, q0);
//         }
...
//     for (int k = 0; k < nPolar; ++k)
//     {
//         // int output_idx = sl_idx * (nEvents * nPolar) + event_idx * nPolar + k;
//         int idx = sl_idx * nSLComb * nEvents + event_idx * nPolar + k;
