...
__global__ void computeNLLKernelWeighted(
    const cuComplex *__restrict__ vector,
    ...
    double phsp_factor = 1.0)
{
    ...
    if (group_idx < total_groups)
    {
        ...
        group_contribution = weight * (-log(fmax(group_sum / phsp_factor, epsilon)));
    }
    ...
    double phsp_factor = 1.0)
{
    ...
    if (d_weights != nullptr)
    {
        ...
        computeNLLKernelWeighted<<<gridSize, blockSize, shared_mem_size>>>(
            d_S, d_weights, d_Q, d_final_result,
            nlength, npolar, phsp_factor);
    }
    else
    {
        // 使用原始核函数（向后兼容）
        computeNLLKernel<<<gridSize, blockSize, shared_mem_size>>>(
            d_S, d_Q, d_final_result,
            nlength, npolar, phsp_factor);
    }
...
//     // 调用合并核函数
//     computeNLLKernel<<<gridSize, blockSize, shared_mem_size>>>(d_S, d_Q, d_final_result, nlength, npolar, phsp_factor);
